...
//             //添加物品名和数量   数据对照下面的格式
//             data: nus
//             // 数据数组，name 为数据项名称，value 为数据项值    格式
//             // { value: 335, name: '物品资产1' },
//           }
//         ]
//       };
//       // 使用刚指定的配置项和数据显示图表。
...
(function (){
  ...
  myChart2 = echarts.init(document.getElementById('test'));
  ...
  sum = one(data_value_list);

  // alert(sum)
  // var data_value_list1 = new Array();
  // var data_list1 = [];

  for(var i=0; i<data_list.length; i++){
    data_list[i].value=parseFloat((data_list[i].value/sum*100).toFixed(5));
  }
  for(var i=0; i<data_value_list.length; i++){
    data_value_list[i]=parseFloat((data_value_list[i]/sum*100).toFixed(5));
  }

  // var data_list1 = [];
  // let data_value_list1 = [];
  // for (var i = 0; i < data_list.length; i++){
  //   data_list1.push({
  //     "value":(data_list[i].value/sum*100).toFixed(5)+"%", "name":data_list[i].name
  //   })
  // }
  //
  // for (var i = 0; i < data_value_list.length; i++){
  //   data_value_list1.push(data_value_list[i])
  // }
  ...
  percentage_map = echarts.init(document.getElementById('spanc'));
  percentage_map.setOption(a_option);
})();
        ...
        data: (function () {
          ...
          $.ajax({
            ...
            success:function (result) {
              ...
              male.push(result[0].nums)
              male.push(80)
              male.push(61)
              female.push(result[1].nums)
              female.push(15)
              female.push(20)
            }
          })
...
(function (){
  ...
  option = {
    ...
    angleAxis: {
      max: Math.max.apply(null, data_value_list)+200,
      startAngle: 75
    },
    radiusAxis: {
      type: 'category',
...
//               for (var i = 0; i < result.length; i++){
//                 datas.push({
//                   "value":result[i].count, "name":result[i].type
//                 })
//
//               }
//
        ...
        data: (function () {
          ...
          $.ajax({
            ...
            success:function (result) {
              ...
              for (var i = 0; i < result.length; i++){
                datas.push({
                  "value":result[i].nums, "name":result[i].age_type
                })
...
(function () {
  // 1.实例化对
  var myChart = echarts.init(document.getElementById('ly'));
  // 2.指定配置项和数据
  option = {
    tooltip: {
      trigger: 'axis',
        ...
        data: (function () {
          ...
          $.ajax({
            ...
            success:function (result) {
              ...
              for (var i = 0; i < result.length; i++){
                datas.push({
                  "value":result[i].count, "name":result[i].name
                })
              }
        ...
        data: (function () {
          ...
          $.ajax({
            ...
            success:function (result) {
              ...
              for (var i = 0; i < result.length; i++){
                datas.push({
                  "value":result[i].count, "name":result[i].illness
                })
              }
